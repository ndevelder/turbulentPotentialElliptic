...
void v2ftpFinal::correct()
{
    ...
    // Update Epsilon-hat
    //*************************************// 
    volVectorField wdunit("wdunit", fvc::grad(wdamp_)/(mag(fvc::grad(wdamp_)) + gtph0));  
    
    epsHat_ = epsilon_/(k_ + (cEhmM_*nu()*mag((gradkSqrt_ & wdunit))));
    bound(epsHat_,eH0);	
	...
	// Epsilon-hat Length and TIme scales
    ...
    //beta_ = 1.0 - exp(-cB_*sqrt(phiActual*phiActual/(nu()*epsilon_) + tph0));
    beta_ = min(6.0*sqr(1.0 - alpha_), alpha_/alpha_);
	
	volScalarField nutExact("nutExact", mag(psiActual_)/(mag(vorticity_) + (cNL_/T)));  
	...
	const volScalarField rII("rII", sqrt(phiActual_*phiActual_ + (psiActual_ & psiActual_) + k0_*k0_));
    ...
    volScalarField cp1eqn("cp1eqn", (2.0*cP1_*sqrt(1.0-alpha_ + SMALL)));
    ...
    // Psi Equation
    //*************************************// 
	volVectorField vecProd("vecProd", phiActual_*vorticity_);
	volVectorField curvProd("curvProd", upsilon_*B_);
	volVectorField addedPsiProd("addedPsiProd", tppsi_ & roTen);

    volScalarField fdamp("fdamp", tanh(wdamp_));

    if(cp1Type_.value() == 1.0){
        fdamp = tanh(wdamp_);
    }else if(cp1Type_.value() == 2.0){
        fdamp = tanh(15.0*sqr(1.0-alpha_));
    }else if(cp1Type_.value() == 3.0){
        fdamp = nutFrac();
    }else{
        fdamp = alpha_/alpha_;
    }
    ...
    (
      ...
      - fvm::Sp((cP1_-lambda_)*fdamp/T,tppsi_)
	  + cD2_*nutFrac()*(bph + (uudk-(2.0/3.0)))*tppsi_/T

	  // Extra term for fixing hump recirc zone
      + cP4_*(k_*(1.0-alpha_)/(nu()/cLn_ + sqrt(nutDiff_*nu() + sqr(nut0))))*tppsi_
	  
      //- cP3_*0.2*(1.0 - 0.5*lambda_)*(1.0 - sqrt(IIb))*vorticity_
      - fvm::Sp((cP3_-0.12*lambda_)*tpProd_,tppsi_)

	  // Dissipation 
	  + (1.0 - lambda_)*tppsi_/T      
	  + cP5_*lambda_*(2.0*alpha_-1.0)*tpphi_*vorticity_
    ...
    cMuRe_ = (0.6*(0.12 + (1.0-alpha_)*lambda_)*tpphi_ + 0.4*(tppsi_ & tppsi_))*cN1_ + cMu_*tpphi_*(1.0-cN1_);
    //cMuRe_ = cN1_*(0.5*(0.12 + 0.37*lambda_)*tpphi_ + 0.5*(tppsi_ & tppsi_)) + (1.0-cN1_)*cMu_*tpphi_;
    
    nut_ = cMu_*tpphi_*k_*T; 
    nut_ = min(nut_,nutRatMax_*nu());  
    nut_.correctBoundaryConditions();
    bound(nut_,nut0);   

    nutDiff_ = cMuRe_*k_*T; 
    nutDiff_ = min(nutDiff_,nutRatMax_*nu());  
    nutDiff_.correctBoundaryConditions();
    bound(nutDiff_,nut0);     


    //*************************************//
    // Choose EV for heat transfer
    //*************************************//
    ...
    Info<< "Max nut: " << gMax(nut_) << " Max K: " << gMax(k_) << " Max Epsilon: " << gMax(epsilon_) << " Max Phi: " << gMax(phiActual_) <<endl;
